...
def get_verification_token():
    ...
    if not token_input:
        raise RuntimeError("CSRF token not found on Removed page")
...
def fetch_page(start, length, token):
    ...
    raise RuntimeError("RemovedPublic API failed after all retries")
...
def main():
    ...
    if new_rows:
        ...
        print(f"➕ Added {len(new_rows)} new removed providers to master file")
    else:
        print("ℹ️ No new removed providers to add to master file")
